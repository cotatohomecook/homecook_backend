...
public interface ShopRepository extends JpaRepository<Shop, Long> {
    ...
    @Query(value = "SELECT s.*," +
            ...
            "FROM shop s LEFT JOIN order_history oh ON oh.shop_id = s.shop_id " +
...
//    List<ShopSearchResponseInterface> findAllByShopNameOrderByOrderCount(double userLatitude, double userLongitude, String shopName);
//
//    @Query(value = "SELECT s.*, " +
//            "            COALESCE(ROUND(AVG(r.rating), 1), 0) as rating, " +
//            "            COALESCE(COUNT(r.review_id), 0) as reviewCount, " +
...
//    List<ShopSearchResponseInterface> findAllByShopNameOrderByDistance(double userLatitude, double userLongitude, String shopName);
}
